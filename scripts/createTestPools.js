...
async function createTestPool(name) {
  ...
  const _rewardToken = data.AstroToken; //IBEP20
  ...
  const SousPool = await AstroStakingFactory.deployPool(
    _stakedToken,
    _rewardToken,
    _rewardPerSecond,
    _startTimestamp,
  ...
  console.log("Astro NewSousChefContract", event.args.sousChef);
  console.log("create Astro Pool " + name);
