...
contract ZOLAStakingPoolInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when ZOLAYU mining ends.
    uint256 public bonusEndTimestamp;

    // The block number when ZOLAYU mining starts.
    ...
    // ZOLAYU tokens created per block.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > lastRewardTimestamp && stakedTokenSupply != 0) {
            ...
            uint256 zolaReward = multiplier.mul(rewardPerSecond);
            uint256 adjustedTokenPerShare =
            accTokenPerShare.add(zolaReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardTimestamp, block.timestamp);
        uint256 zolaReward = multiplier.mul(rewardPerSecond);
        accTokenPerShare = accTokenPerShare.add(zolaReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
        lastRewardTimestamp = block.timestamp;
    }
